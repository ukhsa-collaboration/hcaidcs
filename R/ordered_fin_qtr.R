...
ordered_fin_qtr <- function(data, quarter_in_long_format){
  ...
  z <- data %>% dplyr::mutate_(temp_year = lazyeval::interp(~substr(var, nchar(var)-4, nchar(var)), var = as.name(quarter_in_long_format) ),
                        text_qtr = lazyeval::interp(~substr(var, 1, nchar(var)-5), var = as.name(quarter_in_long_format) )) %>%
