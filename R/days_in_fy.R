...
days_in_fy <- function(x){
  ...
  year_two <- ifelse(year_one < lubridate::ymd("1999-04-01"),
                     paste0("31-03-19", substr(x, 5,6)),
                     paste0("31-03-20", substr(x, 5,6))
  )
  year_two <- lubridate::dmy(year_two)

  z <- as.numeric(year_two - year_one) + 1
  return(z)
}
